...
# 如果你不用安装额外的库，下面一段话可以不看
# 如果你需要安装一些库， 例如  websocket 库，你可以
# install_lib(lib_name="websocket",lib_install_name="websocket-client")
# lib_name 为 import时的名字,lib_install_name 为 pip install 时的名字
...
def plugin(api:API):
    ...
    # 不要太过频繁的调用它
    response=api.do_get_player_scoreboard(cb=None)
    ...
    response=api.listen_player_logout(cb=None,on_player_login_cb=on_player_logout)
    print(response.ack) #True
    ...
    def delay_exec_func(msg1,msg2):
        print("函数被延迟执行了， "+msg1+" "+msg2) # 3秒后 函数被延迟执行了， 啊吧 啊吧吧
